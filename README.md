...
第三次迭代的时候，发现三个文件里有三个版本的 accuracy。改到 V7，已经不记得 V3 和 V4 的区别是什么。session 结束前忘了写交接，第二天打开项目，花了半小时才想起来上次做到哪里。
这些不是粗心，是没有结构的必然结果。
research-docs-skill 是一个面向 Claude Code 的文档管理 skill——实验数字、方法描述、论文叙事各有唯一权威文件，版本有迹可查，进度随时可交接。无论是日常更新、session 交接还是 ARIS 归档，一条指令完成。
