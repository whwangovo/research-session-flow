---
name: docs
description: "为科研项目生成、更新、归档文档。支持子命令：init（初始化）、update（更新）、status（健康检查）、archive（归档）、handoff（生成交接文档）、migrate（迁移旧结构）、log（写开发日志）、aris（归档 ARIS 产出为中文版）。当用户说"写交接"、"handoff"、"交接文档"、"session 结束"、"记录一下进度"、"写个总结"、"下次继续"时，自动执行 handoff 子命令。当用户说"写日志"、"记日志"、"log"、"开发日志"时，自动执行 log 子命令。当用户说"aris"、"归档aris"、"整理aris"时，自动执行 aris 子命令。"
...
- **单一来源**：每类信息只在一个文件里出现。实验数字 → results.md；方法描述 → methods/；论文叙事 → paper-plan.md；项目状态 → overview.md。其他文档用链接引用，不复制内容。
- **不破坏现有内容**：更新文档时保留原有结构，只修改过期部分
- **git 优先**：移动文件时优先用 `git mv`，保留历史
- **确认再执行**：Archive 和 Migrate 操作在执行前必须等待用户确认
- **README 始终同步**：任何文件增删改都要更新 `docs/README.md`
- **frontmatter 必须**：新建或更新的文档必须包含 frontmatter
- **不创建不必要的目录**：`data/`, `deliverables/` 等只在项目确实需要时创建
