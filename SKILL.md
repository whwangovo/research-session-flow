...
2. **扫描所有 .md 文件**（排除 `archive/` 和 `handoffs/resolved/`）
...
- 活跃 handoff：N 个（最旧：YYYY-MM-DD），已归档：M 个（在 `handoffs/resolved/`）
- 建议：[具体建议]
```
...
**目标**：在 session 结束时生成交接文档，并自动清理已完成的旧 handoff。

### 步骤

1. **环境引导**（确保后续所有步骤可安全执行，不再需要逐步检查）：

   a. **探测 handoff 目录**——按优先级匹配第一个存在的目录：
      ```bash
      HANDOFF_DIR=""
      for candidate in docs/handoffs docs/05-handoff docs/handoff; do
        [[ -d "$candidate" ]] && HANDOFF_DIR="$candidate" && break
      done
      [[ -z "$HANDOFF_DIR" ]] && HANDOFF_DIR="docs/handoffs"
      RESOLVED_DIR="$HANDOFF_DIR/resolved"
      ```
      如果命中旧结构目录（`docs/05-handoff` 或 `docs/handoff`），输出提示：
      `[handoff] 检测到旧目录 {candidate}，本次使用该目录。建议运行 /docs migrate 迁移到标准结构。`

   b. **创建缺失目录**：
      ```bash
      mkdir -p "$HANDOFF_DIR" "$RESOLVED_DIR"
      ```
      `docs/` 本身不存在时 `mkdir -p` 会一并创建。不运行完整 init——handoff 只需要写入目录。

   c. **探测 CLAUDE.md 状态**（只记录，不修改——修改在步骤 10 统一处理）：
      ```
      CLAUDE_MD_STATE = 以下之一：
        "missing"      — CLAUDE.md 不存在
        "has-doc"      — 存在且有 ## Documentation
        "has-handoff"  — 存在且有 ## Last Handoff（但无 ## Documentation）
        "no-section"   — 存在但两个 section 都没有
      ```

   环境引导完成后，`HANDOFF_DIR`、`RESOLVED_DIR`、`CLAUDE_MD_STATE` 三个变量确定，后续步骤直接使用，不再做存在性检查。

2. **获取时间戳**：`date +%Y-%m-%d-%H%M`

3. **回顾活跃 handoff**（每次写 handoff 前必须执行）：
   a. 列出 `$HANDOFF_DIR/*.md`（不含 `resolved/` 子目录），如果没有活跃 handoff 则跳过本步骤
   b. 对每个活跃 handoff，读取其「下一步」章节中的待办项
   c. 对比当前 git log（`git log --oneline -20`）、项目文件状态、本次 session 的工作内容，判断每条待办是否已完成
   d. 如果一个 handoff 的所有「下一步」待办均已完成：
      - 将其 frontmatter `status` 改为 `resolved`
      - `git mv` 到 `$RESOLVED_DIR/`（非 git 仓库用普通 `mv`）
   e. 输出回顾结果摘要（哪些被 resolve 了、为什么），例如：
      ```
      [handoff 回顾] resolved 2 个：
        · 2026-04-15-1430-auth-fix.md — "下一步"已全部完成
        · 2026-04-12-0900-data-pipeline.md — "下一步"已全部完成
      [handoff 回顾] 仍活跃 1 个：2026-04-17-1100-model-tuning.md
      ```

4. **重建 session 内容**（主要来源）：从对话历史重建，不要让用户自己总结。关注：
   - 读取、编辑或创建了哪些文件
   - 运行了哪些命令及其结果
   - 遇到了什么问题、如何解决的
   - 用户说想要什么 vs 实际交付了什么
...
6. **读取当前项目状态**（补充来源，均为可选——有什么读什么）：
   - `docs/evaluation/results.md`（如有）
   - `docs/project/overview.md`（如有）
   - 最近修改的文件（`git status`，如果是 git 仓库）
   - 如果以上均不存在，跳过本步骤，仅依赖步骤 4-5 的信息

7. **生成 slug**：从 session 主题提取 2-4 个关键词，用 `-` 连接（中文转拼音首字母或用英文关键词）。如果 session 主题不明确，从 git log 最近提交信息提取。

8. **Git commit（写文档前）**：在生成交接文档之前，先提交当前工作区的改动：
   ```bash
...
9. **生成 `$HANDOFF_DIR/YYYY-MM-DD-HHMM-{slug}.md`**：
...
{根据 git log 和文件变更总结}
{如果步骤 3 中 resolve 了旧 handoff，在此注明解决了哪些旧待办}
...
{写给下一个 session 的指令，格式：先读什么 → 跑什么确认状态 → 再做什么。例如：}
{1. 读 `src/model.py:120-150` 了解当前 attention 实现}
{2. 跑 `python eval.py --quick` 确认 baseline 指标}
{3. 实现 multi-head 变体并对比}
...
10. **更新 CLAUDE.md**：根据步骤 1c 探测到的 `CLAUDE_MD_STATE`，维护 `## Last Handoff` 节。

    **Last Handoff 节内容**（所有分支共用）：
    ```markdown
    ## Last Handoff
    [{slug} ({YYYY-MM-DD})]($HANDOFF_DIR/YYYY-MM-DD-HHMM-slug.md) — {一句话概括下一步}
    ```
    如果有多个活跃 handoff，每个一行。步骤 3 中 resolve 掉的 handoff 从此节移除。

    **按 `CLAUDE_MD_STATE` 分支处理**：

    | 状态 | 操作 |
    |------|------|
    | `missing` | 创建 CLAUDE.md，仅写入 `## Last Handoff` 节。不生成其他内容——完整 CLAUDE.md 应由用户或 `/docs init` 创建 |
    | `no-section` | 在 CLAUDE.md 末尾追加空行 + `## Last Handoff` 节 |
    | `has-doc` | 在 `## Documentation` 节之后、下一个 `##` 之前插入或替换 `## Last Handoff` 节 |
    | `has-handoff` | 直接替换已有的 `## Last Handoff` 节内容 |
...
| `status` / `archive` / `log` | 不修改 CLAUDE.md |
| `handoff` | 更新 `## Last Handoff` 节（活跃 handoff 摘要 + 链接；resolve 时清理）。CLAUDE.md 不存在时创建最小版本；无锚点节时追加到末尾 |

**同步规则**：
- 只更新，不重写——用 Edit 精确替换对应字段，不动其他内容
- 内容来源：从 `docs/project/overview.md` 或 `docs/evaluation/results.md` 提取摘要，压缩成 1-2 行
- 不降级：如果 CLAUDE.md 里的字段比 docs 更新，保留 CLAUDE.md 的版本（例外：`## Last Handoff` 节每次完整替换，以反映最新活跃状态）
