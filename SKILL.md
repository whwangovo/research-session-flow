...
2. **回顾活跃 handoff**（每次写 handoff 前必须执行）：
   a. 列出 `docs/handoffs/*.md`（不含 `resolved/` 子目录），如果没有活跃 handoff 则跳过本步骤
   ...
   d. 如果一个 handoff 的所有「下一步」待办均已完成：
      ...
      - `git mv` 到 `docs/handoffs/resolved/`（非 git 仓库用普通 `mv`；目录不存在则先 `mkdir -p`）
...
3. **重建 session 内容**（主要来源）：从对话历史重建，不要让用户自己总结。关注：
   - 读取、编辑或创建了哪些文件
   - 运行了哪些命令及其结果
   - 遇到了什么问题、如何解决的
   - 用户说想要什么 vs 实际交付了什么
...
6. **生成 slug**：从 session 主题提取 2-4 个关键词，用 `-` 连接（中文转拼音首字母或用英文关键词）。如果 session 主题不明确，从 git log 最近提交信息提取。

7. **Git commit（写文档前）**：在生成交接文档之前，先提交当前工作区的改动：
   ```bash
...
8. **生成 `docs/handoffs/YYYY-MM-DD-HHMM-{slug}.md`**：
...
{根据 git log 和文件变更总结}
{如果步骤 2 中 resolve 了旧 handoff，在此注明解决了哪些旧待办}
...
9. **更新 CLAUDE.md**：在 CLAUDE.md 中维护一个 `## Last Handoff` 节，内容为当前活跃 handoff 的一句话摘要 + 文件链接。如果该节不存在则创建（放在 `## Documentation` 节之后）；如果已存在则替换内容。格式：
   ```markdown
   ## Last Handoff
   [{slug} ({YYYY-MM-DD})](docs/handoffs/YYYY-MM-DD-HHMM-slug.md) — {一句话概括下一步}
   ```
   如果有多个活跃 handoff，每个一行。步骤 2 中 resolve 掉的 handoff 从此节移除。
...
| `handoff` | 更新 `## Last Handoff` 节（活跃 handoff 摘要 + 链接；resolve 时清理） |
